...
> **상태(2026-09-12)**: 2027학년도 수시 접수 종료. 예약 수집 중지(PC 작업 `susi-ratio-feed` 비활성, 워크플로 cron 주석).
> 최종 182/207개교, 스냅샷 181회차. 산출물은 `export/`(최종 요약 xlsx, 시간대별 xlsx/csv).
> 정시에 재사용하려면: `universities.json` 링크를 정시 페이지로 교체 → 워크플로 cron 복원 → `schtasks /Change /TN susi-ratio-feed /ENABLE`.
...
서울대는 경쟁률 페이지 대신 입학본부 공지사항에 「수시모집 지원서 접수현황(MM/DD HH:MM기준)」 PDF를 하루 몇 차례 첨부한다.
`snu_collect.py`가 최신 공지의 첨부를 받아(Referer 필요) 숫자 열을 좌표로 읽고, 행 순서 템플릿(S1·S2·S3)에 대응시켜
`data/extra/서울대학교.json`을 만들면 `collect.py`가 대학명으로 병합한다(feed.cmd가 먼저 실행). 소계·총계 합이 맞지 않으면
ok=False로 떨어지므로, 서울대가 표 구성을 바꾸면 템플릿을 고쳐야 한다. 로컬 검증: `python snu_collect.py --pdf 파일.pdf`.
